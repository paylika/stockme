-- ============================================================
-- StockMe — Avis et notes des produits (étoiles + photos + commentaire)
--
-- À coller dans Supabase → SQL Editor → Run.
--
-- Les règles retenues :
--   • tout utilisateur CONNECTÉ peut noter un produit (de 1 à 5 étoiles),
--     écrire un commentaire et joindre jusqu'à 3 photos ;
--   • UN SEUL avis par personne et par produit (il peut le modifier) ;
--   • on ne peut PAS noter son propre produit (anti-triche) ;
--   • les avis sont publics (visibles par tout le monde, comme sur Alibaba) ;
--   • la moyenne et le nombre d'avis sont recalculés automatiquement dans
--     `products.rating_avg` / `products.rating_count` → affichage instantané
--     sur les cartes du catalogue, sans calcul à chaque page.
--
...
-- Tout le monde peut LIRE les avis (c'est ce qui crée la confiance).
...
-- Écrire : son propre avis, et jamais sur son propre produit.
...
-- ------------------------------------------------------------------
-- 3. Moyenne et nombre d'avis sur le produit (recalcul automatique)
-- ------------------------------------------------------------------
...
-- Rattrapage : recalculer les produits qui ont déjà des avis.
UPDATE public.products p
...
-- ------------------------------------------------------------------
-- 4. Lecture groupée : moyenne + répartition + liste (avec l'auteur)
-- ------------------------------------------------------------------
...
-- CONTRÔLES
-- ============================================================
-- A) La table et les colonnes existent ?
-- select to_regclass('public.product_reviews') as table_avis,
--        (select count(*) from information_schema.columns
--          where table_name='products' and column_name in ('rating_avg','rating_count')) = 2 as colonnes_notes;
--
-- B) Les avis enregistrés
...
--
-- C) La moyenne est-elle bien reportée sur le produit ?
-- select name, rating_avg, rating_count from public.products
--  where rating_count > 0 order by rating_count desc limit 20;
